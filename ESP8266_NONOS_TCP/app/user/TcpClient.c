...
struct espconn *client_tcp_espconn = NULL;
void send_tcp_client(uint8_t *buf, uint8_t len)
{
	espconn_send(client_tcp_espconn, buf, len);
}
...
void Check_WifiState(void)
{
	...
	if (getState == STATION_GOT_IP)
	{
		...
		struct ip_info info;
		const char local_ip[4] = {192, 168, 5, 1};	//目标IP地址（广播）
		const char remote_ip[4] = {192, 168, 4, 1}; //目标IP地址,必须要先从手机获取，否则连接失败.
		wifi_get_ip_info(STATION_IF, &info);		//查询 WiFi模块的 IP 地址

		my_station_init((struct ip_addr *)remote_ip, (struct ip_addr *)local_ip, 8266); //连接到目标服务器的6000端口
	}
}
...
void tcp_client_init() //初始化
{
	...
	os_strcpy(stationConf.ssid, "DAPAP");		 //改成你自己的   路由器的用户名
	os_strcpy(stationConf.password, "99999999"); //改成你自己的   路由器的密码
