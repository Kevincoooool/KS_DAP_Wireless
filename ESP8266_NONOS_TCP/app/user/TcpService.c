...
struct espconn *tcp_espconn = NULL;
...
void send_tcp_server(uint8_t *buf, uint8_t len)
{
	espconn_send(tcp_espconn, buf, len);
}
...
void Inter213_InitTCP(uint32_t Local_port)
{
	...
	espconn_regist_time(&user_tcp_espconn, 7200, 0);			//设置超时断开时间 单位：秒，最大值：7200 秒
}
...
void WIFI_Init()
{
	...
	apConfig.ssid_len = os_strlen("DAPAP");	  //设置ssid长度
