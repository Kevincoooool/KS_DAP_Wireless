-- TeenyUSB not support multiple config
return Device {
    strManufacturer = "TeenyDT",
    strProduct = "TeenyDT Multi Config DEMO",
   ...
   Config {
        ...
        Interface{
            ...
            bInterfaceSubClass = 0x06,     -- SCSI
            bInterfaceProtocol = 0x50,     -- BOT
            EndPoint(IN(2),  BulkDouble, 64),
            EndPoint(OUT(2), BulkDouble, 64),
        },
        USB_HID{
            ReadEp = EndPoint(IN(3),  Interrupt, 16),
            WriteEp = EndPoint(OUT(3), Interrupt, 16),
            report = HID_InOut(16),
        },
   },
}