...
void oled_show(void)
{
	...
	if (Show.low_power) //遥控低电量
	{
		...
		for (uint8_t i = 0; i < 12; i++)
			OLED_Show_CH_String(Line1_Begin + i * 6, Y0, oled_CH16[i], 12, show_mode);
	}
	else if (Show.test_flag & BIT5) //飞机低电量
	{
		...
		for (uint8_t i = 0; i < 12; i++)
			OLED_Show_CH_String(Line1_Begin + i * 6, Y0, oled_CH17[i], 12, show_mode);
	}
	else if (Show.Set_Succeed) //设置成功
	{
		show_mode = 1;
		for (uint8_t i = 0; i < 12; i++)
			OLED_Show_CH_String(Line1_Begin + i * 6, Y0, oled_CH21[i], 12, show_mode);
	}
	else if (Show.Set_Fail) //设置失败
	{
		show_mode = 1;
		for (uint8_t i = 0; i < 12; i++)
			OLED_Show_CH_String(Line1_Begin + i * 6, Y0, oled_CH22[i], 12, show_mode);
	}
	else if (Show.test_flag & BIT8) //指南针异常！
	{
		...
		for (uint8_t i = 0; i < 12; i++)
			OLED_Show_CH_String(Line1_Begin + i * 6, Y0, oled_CH24[i], 12, show_mode);
	}
	else
	{
	...
	//////////////////////////////////////////////////////
	if (0)
		OLED_Show_CH(2, Y0, 6, 12, 1); //无线模块故障显示X
	else
		OLED_ShowNumber(2, Y0, 2, 3, 12); //显示无线信道
	...
	{
	case 0:
		OLED_Show_CH(Line1_Begin + 85, Y0, 6, 12, 1);
		break;
	case 1:
		OLED_Show_CH(Line1_Begin + 85, Y0, 7, 12, 1);
		break;
	case 2:
		OLED_Show_CH(Line1_Begin + 85, Y0, 8, 12, 1);
		break;
	case 3:
		OLED_Show_CH(Line1_Begin + 85, Y0, 9, 12, 1);
		break;
	case 4:
		OLED_Show_CH(Line1_Begin + 85, Y0, 10, 12, 1);
		break;
	default:
		OLED_Show_CH(Line1_Begin + 85, Y0, 11, 12, 1);
		break;
	}
	///////////////////////////////第二行///////////////////////////////////
	//显示电压
	...
	OLED_ShowNumber(Line2_Begin + 30, Y1, 80 / 100, 1, 12);
	OLED_ShowNumber(Line2_Begin + 42, Y1, 80 % 100, 2, 12);
	if (Show.Battery_Rc % 100 < 10)
		OLED_ShowNumber(Line2_Begin + 42, Y1, 0, 1, 12);

	OLED_ShowString(Line2_Begin + 64, Y1, "F-V:", 12, 1);
	OLED_ShowString(Line2_Begin + 100, Y1, ".", 12, 1);
	OLED_ShowNumber(Line2_Begin + 94, Y1, 70 / 100, 1, 12);
	OLED_ShowNumber(Line2_Begin + 106, Y1, 70 % 100, 2, 12);
	if (Show.Battery_Fly % 100 < 10)
		OLED_ShowNumber(Line2_Begin + 106, Y1, 0, 1, 12);
	///////////////////////////////第三、四行/////////////////////////////////
	//显示遥控数据
	...
	if (Show.Rc_num && Show.hardware_type == 1)
	{
		...
		//显示姿态角度和高度
		if (Show.X < 0)
			temp1 = -Show.X, OLED_ShowString(X_Begin, Y4, "-", 12, 1);
		else
			temp1 = Show.X, OLED_ShowString(X_Begin, Y4, "+", 12, 1);
		...
		if (Show.Y < 0)
			temp1 = -Show.Y, OLED_ShowString(Y_Begin, Y4, "-", 12, 1);
		else
			temp1 = Show.Y, OLED_ShowString(Y_Begin, Y4, "+", 12, 1);
		...
		if (Show.H < 0)
			temp1 = -Show.H / 10, OLED_ShowString(Z_Begin, Y4, "-", 12, 1);
		else
			temp1 = Show.H / 10, OLED_ShowString(Z_Begin, Y4, "+", 12, 1);
		//最高显示9999分米,也就是999.9米。
		if (temp1 > 9999)
			temp1 = 9999, OLED_ShowString(Z_Begin, Y4, ">", 12, 1);
...
void OLED_Show_Seting(void)
{
	...
	for (uint8_t i = 0; i < 8; i++)
		OLED_Show_CH_String(40 + i * 6, cnt, oled_CH0[i], 12, 1);
	...
	case 0:
		...
		for (uint8_t i = 0; i < 10; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH1[i], 12, 1);
		if (set_bit == bit_cnt)
			mode = 0;
		else
			mode = 1;
		if (set_temp & BIT0)
			for (uint8_t i = 0; i < 6; i++)
				OLED_Show_CH_String(94 + i * 6, cnt, oled_CH12[i], 12, mode);
		else
			for (uint8_t i = 0; i < 6; i++)
				OLED_Show_CH_String(94 + i * 6, cnt, oled_CH13[i], 12, mode);
		bit_cnt++; //bit_max=0

		cnt += 13;
		for (uint8_t i = 0; i < 10; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH2[i], 12, 1);
		if (set_bit == bit_cnt)
			mode = 0;
		else
			mode = 1;
		...
		cnt += 13;
		for (uint8_t i = 0; i < 10; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH3[i], 12, 1);
		if (set_bit == bit_cnt)
			mode = 0;
		else
			mode = 1;
		...
		cnt += 13;
		for (uint8_t i = 0; i < 10; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH4[i], 12, 1);
		if (set_bit == bit_cnt)
			mode = 0;
		else
			mode = 1;
	...
	case 1:
		cnt += 13;
		for (uint8_t i = 0; i < 10; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH14[i], 12, 1);
		if (set_bit == bit_cnt)
			mode = 0;
		else
			mode = 1;
		...
		cnt += 13;
		for (uint8_t i = 0; i < 10; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH18[i], 12, 1);
		if (set_bit == bit_cnt)
			mode = 0;
		else
			mode = 1;
		...
		cnt += 13;
		for (uint8_t i = 0; i < 10; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH20[i], 12, 1);
		if (set_bit == bit_cnt)
			mode = 0;
		else
			mode = 1;
...
void OLED_Show_Test(void)
{
	...
	for (uint8_t i = 0; i < 10; i++)
		OLED_Show_CH_String(34 + i * 6, cnt, oled_CH5[i], 12, 1);
	...
	case 0:
		cnt += 13;
		for (uint8_t i = 0; i < 8; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH6[i], 12, 1);
		if (test_bit == bit_cnt++)
			mode = 0;
		else
			mode = 1;
		if (test_temp & BIT0)
			for (uint8_t i = 0; i < 4; i++)
				OLED_Show_CH_String(104 + i * 6, cnt, oled_CH10[i], 12, mode);
		else
			for (uint8_t i = 0; i < 4; i++)
				OLED_Show_CH_String(104 + i * 6, cnt, oled_CH11[i], 12, mode);

		cnt += 13;
		for (uint8_t i = 0; i < 8; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH7[i], 12, 1);
		if (test_bit == bit_cnt++)
			mode = 0;
		else
			mode = 1;
		...
		cnt += 13;
		for (uint8_t i = 0; i < 10; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH8[i], 12, 1);
		if (test_bit == bit_cnt++)
			mode = 0;
		else
			mode = 1;
		...
		cnt += 13;
		for (uint8_t i = 0; i < 8; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH23[i], 12, 1);
		if (test_bit == bit_cnt++)
			mode = 0;
		else
			mode = 1;
	...
	case 1:
		cnt += 13;
		for (uint8_t i = 0; i < 10; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH19[i], 12, 1);
		if (test_bit == bit_cnt++)
			mode = 0;
		else
			mode = 1;
		...
		for (uint8_t i = 0; i < 14; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH15[i], 12, 1);
		if (test_bit == bit_cnt++)
			mode = 0;
		else
			mode = 1;
		...
		for (uint8_t i = 0; i < 9; i++)
			OLED_Show_CH_String(0 + i * 6, cnt, oled_CH25[i], 12, 1);
		if (test_bit == bit_cnt++)
			mode = 0;
		else
			mode = 1;
...
void Gesture_Check(void)
{
	...
	if (temp != Show.windows)
	{
		...
		if (temp == 1 && set_temp != Show.set_flag)
			send_flag = 1;
